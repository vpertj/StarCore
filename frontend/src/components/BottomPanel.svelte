...
<div
  ...
  class:hidden={!$bottomPanelVisible}
>
    ...
    <div class="header-actions">
      {#if $bottomPanelTab === "terminal"}
        ...
        <button
          ...
          onclick={() => {
            if ($activeTerminalId && window.backend)
              window.backend.TerminalWrite($activeTerminalId, "\x1b[2J\x1b[H");
          }}
        >
          <svg
            viewBox="0 0 16 16"
    ...
    {#if $bottomPanelTab === "terminal"}
      {#if $terminalTabs.length > 0}
        {#each $terminalTabs as termTab (termTab.id)}
          <div
            class="terminal-pane"
            class:hidden={$activeTerminalId !== termTab.id}
          >
