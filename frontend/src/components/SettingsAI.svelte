<script>
  ...
  let editExistingModels = $derived(
    $customModels.filter((m) => m.providerId === editProviderId),
  );
  let editExistingModelIds = $derived(
    new Set(editExistingModels.map((m) => m.modelId || m.id.split(":").pop())),
  );
  let editNewModels = $derived(
    editFetchedModelList.filter((m) => !editExistingModelIds.has(m)),
  );
  ...
  function openEditProvider(pid) {
    ...
    editEndpoint =
      existing?.endpoint ||
      builtinProviders.find((p) => p.id === pid)?.defaultEndpoint ||
      "";
    ...
    try {
      ...
      if (!resp.ok) {
        if (resp.status === 401 || resp.status === 403)
          fetchModelError = `认证失败 (HTTP ${resp.status})：API 密钥无效`;
        else if (resp.status >= 500)
          fetchModelError = `服务端错误 (HTTP ${resp.status})`;
        else fetchModelError = `获取模型列表失败 (HTTP ${resp.status})`;
        return;
      }
      const data = await resp.json();
      const items = data.data || data.models || [];
      fetchedModelList = items
        .map(function (/** @type {any} */ m) {
          return typeof m === "string" ? m : m.id || m.name;
        })
        .filter(Boolean);
      if (fetchedModelList.length === 0) fetchModelError = "未获取到可用模型";
    } catch (/** @type {any} */ e) {
      let errMsg = (e.message || String(e)).replace(
        /sk-[a-zA-Z0-9]{8,}/g,
        "sk-****",
      );
      ...
      <thead>
        <tr style="background-color: var(--bg-secondary);">
          <th
            ...
            >{$t("settings.ai.model") || "模型"}</th
          >
          <th
            class="text-left px-4 py-2 font-medium text-xs"
            style="color: var(--text-secondary);"
            >{$t("settings.ai.provider") || "服务商"}</th
          >
          <th
            class="text-center px-2 py-2 font-medium text-xs"
            style="color: var(--text-secondary);">Context</th
          >
          <th
            class="text-center px-4 py-2 font-medium text-xs"
            style="color: var(--text-secondary);"
            >{$t("settings.ai.enabled") || "启用"}</th
          >
          <th
            class="text-center px-4 py-2 font-medium text-xs"
            style="color: var(--text-secondary);"
          ...
          {#each providerModelList as model}
            ...
            <tr class="border-t" style="border-color: var(--border);">
              <td class="px-4 py-2" style="color: var(--text-primary);"
                >{model.name}{#if model.supportsThinking}<span
                    class="ml-1 text-[10px] px-1 rounded"
                    style="background-color: var(--border); color: var(--text-secondary);"
                    >thinking</span
                  >{/if}</td
              >
              ...
              >
              <td
                class="text-center px-2 py-2 text-[10px]"
                style="color: var(--text-muted);"
                >{#if (model.maxTokens || 0) >= 1000000}{Math.round(
                    (model.maxTokens || 0) / 10000,
                  ) / 100}M{:else if (model.maxTokens || 0) >= 1000}{Math.round(
                    (model.maxTokens || 0) / 1000,
                  )}K{:else}{model.maxTokens || 0}{/if}</td
              >
              <td class="text-center px-4 py-2">
                <div class="flex justify-center">
                  <button
                    class="relative w-8 h-4 rounded-full transition-colors"
                    style="background-color: {modelEnabled
                      ? 'var(--accent)'
                      : 'var(--border)'};"
                    onclick={() => setModelEnabled(modelKey, !modelEnabled)}
                    title={$t("settings.ai.enabled") || "启用"}
                  >
                    <span
                      class="absolute top-0.5 w-3 h-3 rounded-full transition-transform"
                      style="background-color: #ffffff; left: {modelEnabled
                        ? 'calc(100% - 14px)'
                        : '2px'};"
                    ></span>
                  </button>
              ...
              <td class="text-center px-4 py-2">
                <div class="flex items-center justify-center gap-1">
                  <button
                    ...
                    style="color: var(--text-secondary);"
                    onclick={() =>
                      testModelConnection(modelKey, provider.id, "", "")}
                  ...
                  {#if testModelResults[modelKey]}
                    ...
                    <span
                      style="color: {r.ok
                        ? '#2ea043'
                        : '#f14c4c'}; font-size: 10px; max-width: 80px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;"
                      title={r.msg}>{r.ok ? "✓" : "✗"} {r.msg}</span
                    >
                  {/if}
                  <button
                    class="p-1 rounded transition-colors hover:bg-white/10"
        ...
        {#if $customModels.length > 0}
          ...
          {#each $customModels as cm}
            ...
            <tr class="border-t" style="border-color: var(--border);">
              ...
              >
              <td class="px-4 py-2" style="color: var(--text-secondary);"
                >{cm.providerName || cm.providerId}</td
              >
              <td
                class="text-center px-2 py-2 text-[10px]"
                style="color: var(--text-muted);"
                >{#if cmCtx >= 1000000}{Math.round(cmCtx / 10000) /
                    100}M{:else if cmCtx >= 1000}{Math.round(
                    cmCtx / 1000,
                  )}K{:else if cmCtx > 0}{cmCtx}{:else}—{/if}</td
              >
              <td class="text-center px-4 py-2">
                <div class="flex justify-center">
                  <button
                    class="relative w-8 h-4 rounded-full transition-colors"
                    style="background-color: {cm.enabled !== false
                      ? 'var(--accent)'
                      : 'var(--border)'};"
                  ...
                  >
                    <span
                      class="absolute top-0.5 w-3 h-3 rounded-full transition-transform"
                      style="background-color: #ffffff; left: {cm.enabled !==
                      false
                        ? 'calc(100% - 14px)'
                        : '2px'};"
                    ></span>
              ...
              <td class="text-center px-4 py-2">
                <div class="flex items-center justify-center gap-1">
                  ...
                  <button
                    class="p-1 rounded transition-colors hover:bg-white/10"
                    style="color: var(--text-secondary);"
                    onclick={() => openEditCustomModel(cm)}
                    title="Edit"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
...
{#if showAddModelDialog}
  <div
    ...
    onclick={(e) => {
      if (e.target === e.currentTarget) showAddModelDialog = false;
    }}
    role="button"
    tabindex="-1"
    onkeydown={(e) => {
  ...
  >
    <div
      ...
      style="width: 480px; background-color: var(--bg-primary); border: 1px solid var(--border);"
    >
      ...
      <div class="space-y-4">
        <div>
          <label
            for="add-model-provider"
            class="block text-xs mb-1.5 font-medium"
            style="color: var(--text-secondary);"
            >{$t("settings.ai.provider") || "服务商"}</label
          >
          <select
            id="add-model-provider"
            bind:value={addModelProvider}
        ...
        <div>
          <label
            for="add-model-provider-name"
            class="block text-xs mb-1.5 font-medium"
            style="color: var(--text-secondary);">供应商名称</label
          >
          <input
            id="add-model-provider-name"
            type="text"
            bind:value={addModelProviderName}
            placeholder="OpenAI"
            class="w-full px-3 py-2 rounded border text-sm"
            style="background-color: var(--bg-secondary); color: var(--text-primary); border-color: var(--border);"
          />
...
{#if showEditProviderDialog}
  <div
    ...
    style="background-color: rgba(0, 0, 0, 0.5);"
    onclick={(e) => {
      if (e.target === e.currentTarget) showEditProviderDialog = false;
    }}
    role="button"
    tabindex="-1"
    onkeydown={(e) => {
      ...
      <div class="space-y-4">
        ...
        {#if editExistingModels.length > 0}
          <div>
            ...
            <label
              class="block text-xs mb-1.5 font-medium"
              style="color: var(--text-secondary);"
              >已添加 ({editExistingModels.length})</label
            >
            <div
              class="rounded border overflow-y-auto"
              ...
              {#each editExistingModels as em}
                ...
                >
                  ...
                  <button
                    ...
                    style="color: #f14c4c;"
                    onclick={() => deleteCustomModel(em.id)}
                    title="Remove"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
            ...
            >
              {#each editNewModels as fm}
                <div
                  ...
                  style="background-color: {editSelectedModelsToAdd.includes(fm)
                    ? '#094771'
                    : 'transparent'}; color: {editSelectedModelsToAdd.includes(
                    fm,
                  )
                    ? '#ffffff'
                    : 'var(--text-primary)'}; border-color: var(--border);"
                  onclick={() => editToggleModelSelection(fm)}
                  role="checkbox"
                  aria-checked={editSelectedModelsToAdd.includes(fm)}
                  tabindex="0"
                  onkeydown={(e) => {
                    if (e.key === "Enter" || e.key === " ") {
                      e.preventDefault();
                      editToggleModelSelection(fm);
                    }
                  }}
                >
                  <span style="width: 16px; text-align: center;"
                    >{editSelectedModelsToAdd.includes(fm) ? "✓" : ""}</span
                  >
                  <span class="truncate">{fm}</span>
                </div>
              {/each}
        ...
        >
          ...
          <button
            ...
            style="background-color: var(--accent); color: #ffffff;"
            onclick={() => {
              setProviderConfig(editProviderId, {
                apiKey: editApiKey,
                endpoint: editEndpoint,
              });
...
{#if showEditCustomModelDialog}
  <div
    ...
    style="background-color: rgba(0, 0, 0, 0.5);"
    onclick={(e) => {
      if (e.target === e.currentTarget) showEditCustomModelDialog = false;
    }}
    role="button"
    tabindex="-1"
    onkeydown={(e) => {
    ...
    >
      ...
      <div class="space-y-4">
        <div>
          ...
          <select
            id="edit-cm-provider"
            bind:value={editCustomModelProvider}
            class="w-full px-3 py-2 rounded border text-sm"
            style="background-color: var(--bg-secondary); color: var(--text-primary); border-color: var(--border);"
          >
            {#each builtinProviders as bp}<option value={bp.id}
                >{bp.name}</option
              >{/each}
          </select>
        </div>
        <div>
          <label
            for="edit-cm-provider-name"
            class="block text-xs mb-1.5 font-medium"
            style="color: var(--text-secondary);">供应商名称</label
          >
          <input
            id="edit-cm-provider-name"
            type="text"
            bind:value={editCustomModelProviderName}
            placeholder="OpenAI"
            class="w-full px-3 py-2 rounded border text-sm"
            style="background-color: var(--bg-secondary); color: var(--text-primary); border-color: var(--border);"
          />
